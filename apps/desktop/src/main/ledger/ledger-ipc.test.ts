...
describe("ledger IPC", () => {
  it("validates list, suggestions, and transfer commands at the boundary", async () => {
    ...
    const service = { list: vi.fn(async () => []), suggestions: vi.fn(async () => []), delete: vi.fn(async () => undefined), linkTransfer: vi.fn(async () => undefined), unlinkTransfer: vi.fn(async () => undefined) };
