import type { ActivityInverse, ActivityLogPort, CommitImportBatchCommand } from "@pwm/application";
import type { CommitImportInput, CommittedBatchView, CreateImportDraftInput, ImportBatchId, ImportDraftSummary, ImportDraftView, SelectedSource, SkipCandidateInput, UpdateCandidateInput, WorkspaceId, PrepareLlmFallbackInput, LlmFallbackSource } from "@pwm/contracts";
...
export class DesktopImportController implements ImportController {
  ...
  prepareLlmFallback(input: PrepareLlmFallbackInput) { if (!this.reviews.prepareLlmFallback) return Promise.reject(new Error("LLM_FALLBACK_UNAVAILABLE")); return this.reviews.prepareLlmFallback(input); }
  cancel(batchId: ImportBatchId) { return this.reviews.cancel(batchId); }
  async commit(input: CommitImportInput) { const request: ImportCommitRequest = { ...input, entries: input.entries.map((entry) => ({ ...entry, postings: entry.postings.map((posting) => ({ ...posting, amountMinor: BigInt(posting.amountMinor), currency: posting.currency as never })) })) }; const result = await this.commits.execute(request); if (this.activity?.append) { const timestamp = new Date().toISOString(); const inverse: ActivityInverse = { kind: "soft-delete-journals", journalIds: result.journalIds as never, expectedVersions: result.journalIds.map(() => 0) }; await this.activity.append({ id: crypto.randomUUID() as never, workspaceId: input.workspaceId as WorkspaceId, kind: "bulk-import", entityType: "import-batch", entityId: input.batchId, summary: `Committed ${result.journalIds.length} journal entr${result.journalIds.length === 1 ? "y" : "ies"}`, createdAt: timestamp, updatedAt: timestamp, version: 0, deletedAt: null, undoable: true, undoneAt: null, dependsOn: [] }, inverse); } return result as CommittedBatchView; }
}
