...
export const ImportDraftViewSchema = z.object({ batchId: ImportBatchIdSchema, sourceSha256: z.string().regex(/^[a-f0-9]{64}$/u), status: ImportBatchStatusSchema, revision: z.number().int().nonnegative(), candidates: z.array(ImportCandidateV1Schema), skippedRawRecordIds: z.array(RawRecordIdSchema), warnings: z.array(z.string()) }).strict();
