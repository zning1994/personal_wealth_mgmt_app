...
function unitOfWork() {
  const commits = new Map<string, ImportCommitResult>(); const sources = new Map<string, ImportCommitResult>(); const journals: unknown[] = []; let runCount = 0;
  const unitOfWork = { run: async <T>(work: (transaction: ImportCommitTransaction) => Promise<T>) => { runCount += 1; return work({ ledger: { saveJournal: async (journal) => { journals.push(journal); } }, imports: { findCommit: async (_workspace, key) => commits.get(key) ?? null, findSourceCommit: async (_workspace, sourceSha256) => sources.get(sourceSha256) ?? null, linkRawRecord: async () => undefined, markCommitted: async (_batch, result, key, sourceSha256) => { commits.set(key, result); if (sourceSha256) sources.set(sourceSha256, result); } } }); } };
  return { unitOfWork, journals, commits, sources, get runCount() { return runCount; } };
}
const request = (idempotencyKey = crypto.randomUUID(), sourceSha256 = "a".repeat(64)) => ({ workspaceId: "018f8f19-2d6a-7b00-8000-000000000099", batchId: crypto.randomUUID(), sourceSha256, idempotencyKey, entries: [{ rawRecordIds: [crypto.randomUUID()], occurredOn: "2026-08-04", description: "Synthetic Market", postings: [{ accountId: crypto.randomUUID(), amountMinor: -1000n, currency: "AED" as Currency }, { accountId: crypto.randomUUID(), amountMinor: 1000n, currency: "AED" as Currency }] }] });
describe("CommitImportBatchCommand", () => {
  ...
  it("rejects a source that was already committed under a new idempotency key", async () => { const state = unitOfWork(); const command = new CommitImportBatchCommand(state.unitOfWork, ids()); const first = await command.execute(request()); await expect(command.execute(request(crypto.randomUUID(), "a".repeat(64)))).rejects.toThrow("IMPORT_SOURCE_ALREADY_COMMITTED"); expect(state.journals).toHaveLength(1); expect(first.journalIds).toHaveLength(1); });
