...
export type ImportCommitRequest = { workspaceId: string; batchId: string; idempotencyKey: string; entries: readonly ImportJournalDraft[] };
...
export interface ImportBatchWriteRepository { findCommit(workspaceId: WorkspaceId, key: string): Promise<ImportCommitResult | null>; linkRawRecord(journalId: JournalEntryId, rawRecordId: RawRecordId): Promise<void>; markCommitted(batchId: ImportBatchId, result: ImportCommitResult, key: string): Promise<void> }
...
export class CommitImportBatchCommand {
  ...
  async execute(request: ImportCommitRequest): Promise<ImportCommitResult> {
    ...
    return this.unitOfWork.run(async ({ ledger, imports }) => {
      ...
      const result: ImportCommitResult = { batchId: request.batchId, journalIds }; await imports.markCommitted(request.batchId as ImportBatchId, result, request.idempotencyKey); return result;
    });
  }
}
